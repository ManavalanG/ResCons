...
'''
Ver 24_test (under development) major modifications:
...
settings_file_name = current_path + "/Rescon_Files/Settings_ResCon_testing.txt"
...
def fasta_filter_by_id():
	...
	total_seqs = 0
	extracted=0
	partial_id_dict= {} 	# this is to help with extraction for Marc IPPases project
			...
			if not partial_ids:
				...
				for partial_id in ids_to_be_filtered:
					if partial_id in title:
						...
						extracted += 1
						if partial_id in partial_id_dict:
							partial_id_dict[partial_id] += 1
						else:
							partial_id_dict[partial_id] = 1
						break
	...
	marc_text = output_path + 'Summary.csv'
	marc_text_handle = open(marc_text, 'w')
	for key in partial_id_dict:
		temp = key.replace('|', ' ')
		marc_text_handle.write("%s,%i\n" %(temp, partial_id_dict[key]))
	marc_text_handle.close()
...
clustal_command_label = label_text(frame_clustal, "   Clustalo Command", 0, 20, 0, 0)
