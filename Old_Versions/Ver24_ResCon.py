...
settings_file_name = "Rescon_Files/Settings_ResCon.txt"
...
def clustalo_webserver_fn():
	...
	parameters_string = '--email mg0010@uah.edu --sequence "%s" --outfile "%s"test' % (SeqQuery_file, Output_Path)
...
#	new_thread(lambda: clustalo_api(parameters_string))

	Aligned_Filename = Output_Path + 'test.aln-clustal.clustal'
...
clustalo_local = Radiobutton(frame2, text= 'Use Web Server    ', variable= clustalo_source, value=1)
clustalo_local.grid(row=6, column=1, sticky=E)

higher_e_radio = Radiobutton(frame2, text= 'Use Local Clustalo', variable= clustalo_source, value=2)
higher_e_radio.grid(row=7, column=1, sticky=E)
...
def open_file(filename):
	if win_os and not ubuntu_os:
		# os.system('start ' +filename)	# doesn't work if filepath has spaces in it
		os.startfile(filename, 'open')		# works fine even if filepath has spaces in it
	elif ubuntu_os:
