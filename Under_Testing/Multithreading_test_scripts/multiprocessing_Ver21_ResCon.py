...
def verify_filepath_exists(path):
	...
	if win_os and 'Program Files' in dir_path and not ubuntu_os:
...
def raise_enabler(str):
	...
	try:
		global Blast_Submit, Extract_Submit, descr_extractor_Submit, fasta_filter_by_name_Submit, fasta_filter_by_id_Submit
		...
		submit_buttons = [Blast_Submit, Extract_Submit, descr_extractor_Submit,
...
def multi(tool_fn, *q):
	...
	xyz = mp.Process(target=tool_fn, args= (q,))
	print '1'
	xyz.start()
	print '2'
	xyz.join()
...
def genpept_converter(genbank_entry, fasta_id_length_entry, ignore_incomplete_checkboxval, options_dict, genbank_Submit, top, output_genbank_entry, genbank_processing):
...
def top_win():
	...
	if linux_os:
		...
		genbank_Submit = Button(frame_genbank, text='Convert now!', width=12, command= lambda: multi(genpept_converter,genbank_entry, fasta_id_length_entry, ignore_incomplete_checkboxval, options_dict, genbank_Submit, top, output_genbank_entry, genbank_processing), bg='steelblue2')
